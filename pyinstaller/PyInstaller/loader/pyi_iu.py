...
class ImportManager(object):
    ...
    def install(self):
        import __builtin__
         # added by norritt -- hack for gen_py s?it
        __builtin__.__oldimport__ = __builtin__.__import__
        # hack end
    ...
    def importHook(self, name, globals=None, locals=None, fromlist=None, level=-1):
        ...
        # added by norritt -- hack for gen_py s?it
        if name.find('win32com.gen_py.') == 0:
            import win32com             # get/generate the path where the gen_py files are stored
            gen_path = win32com.__gen_path__.rpartition('\\')[0]
            if not gen_path in sys.path:
                sys.path.append(gen_path)
            modname = '.'.join(nmparts[1:])     # needed: the gen_py files are not located in a win32com package
            __oldimport__(modname)      # needed: load the module 
            mod = sys.modules.get(modname)  # needed: get the module for the rest of this process
            ##del(sys.modules[modname])     # assertion error if deleted (file gencach.py in GetModuleForTypelib)
            sys.modules[name] = mod         # needed: register the module under win32com... (mod not found error if not inserted)
        # end hack
